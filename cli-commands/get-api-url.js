...
exec(
  "aws cloudformation describe-stacks --region us-east-1 --profile jawad-dev-cli --query 'Stacks[?StackName==`web-backend`][].Outputs[?OutputKey==`apiURL`].OutputValue' --output text",
  (error, stdout, stderr) => {
    if (error) {
